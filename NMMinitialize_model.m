function nim = NMDinitialize_model( stim_params, mod_signs, NL_types, reg_params, Xtargets, init_filts, spk_NL_type)
%
...
%     <reg_params>: Struct specifying initial regularization parameters
...
%     <spk_NL_type>: string specifying the type of spk NL (either 'exp', 'logexp', 'logistic', or 'linear')
% OUTPUTS:
...
%     spk_NL_type: String specifying the type of spiking NL ('logexp', 'exp', 'logistic', and 'linear' currently supported)
%                  Note that the likelihood function is spiking-NL-specific: 
%                    (logexp, exp) -> Poisson LL
%                    (logistic)    -> Bernoulli LL
%                    (linear)      -> Gaussian LL (mean-squared error)
...
%         NLtype: String specifying the type of upstream NL ('lin','quad','threshlin','threshP','nonpar')
...
%         NOTE THIS FUNCTION WILL NOT INITIALIZE OTHER THAN DEFAULT (absent) spike-history term
...
poss_NL_types = {'lin','threshlin','quad','threshP','nonpar'};
mods = [];
for imod = 1:Nmods
    tmod = []; 
    ...
    tmod.mod_norm = nan;
    tmod.NLx = 0;
		if strcmp(NL_types{imod},'threshP')
			tmod.NLy = 2;  % default power to quadratic
		else
			tmod.NLy = [];
		end
...
if strcmp(spk_NL_type,'logexp')
	spk_NL_params = [0 1 1 0]; %[theta beta alpha r0]
elseif strcmp(spk_NL_type,'exp')
	spk_NL_params = 0; %[theta]
elseif strcmp(spk_NL_type,'linear')
	spk_NL_params = 0; % theta
elseif strcmp(spk_NL_type,'logistic')
	spk_NL_params = [0 1 1 0]; % [theta beta power r0]
else
